...
def rpyxyz2H(rpy, xyz):
    Ht = [[1, 0, 0, xyz[0]], [0, 1, 0, xyz[1]], [0, 0, 1, xyz[2]], [0, 0, 0, 1]]
...
def CheckPointOverlap(pointsA, pointsB, axis):
    ...
    if np.linalg.norm(axis) == 0:
        return True  # if axis is zero vector, consider it as overlap
...
def CheckBoxBoxCollision(pointsA, axesA, pointsB, axesB):
    ...
    if np.linalg.norm(pointsA[0] - pointsB[0]) > (
        np.linalg.norm(pointsA[0] - pointsA[1])
        + np.linalg.norm(pointsB[0] - pointsB[1])
    ):
        return False
    ...
    #         edge_cross_prods[i, j] = np.cross(
    #             edgesA[i, 1] - edgesA[i, 0], edgesB[j, 1] - edgesB[j, 0]
    #         )
    ...
    # check for edge-edge collisions using cross product of axes
    for i in range(len(axesA)):
        for j in range(len(axesB)):
            edge_cross_prod = np.cross(axesA[i], axesB[j])
            if not CheckPointOverlap(pointsA, pointsB, edge_cross_prod):
                return False
...
if __name__ == "__main__":
    ...
    ref_bbox_pts, ref_bbox_axis = BlockDesc2Points(
        rpyxyz2H(ref_ori, ref_origin), ref_dims
    )
    for i in range(len(test_origins)):
        Hi = rpyxyz2H(test_ori[i], test_origins[i])
        pts_i, ax_i = BlockDesc2Points(Hi, test_dims[i])
        ans_i = CheckBoxBoxCollision(
            np.array(ref_bbox_pts),
            np.array(ref_bbox_axis),
            np.array(pts_i),
            np.array(ax_i),
        )
